...
function mycount {
  ...
  do
    ...
    grep -v "XF:Z:__no_feature" $shdir/priority.sam | grep -v "XF:Z:__not_aligned" | grep -v "^@" | awk '{print $1}' > $shdir/tmpReads.txt
...
function mystat {
  printf 'all_Ensembl\t%i\n' `cat $shdir/htseq-count_$f.txt | grep -v "^__" | awk '{s+=$2} END {print s}'`
...
awk '/^@/ {$1=gensub(/^@/,"",1,$1); print $1 "\t0"}' $out/$f/Unmapped_$f.fq >> $out/$f/forKrona/$f.forKrona.txt

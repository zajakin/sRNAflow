...
server <- function(input, output, session) {
    ...
    output$reports = DT::renderDataTable({
        ...
        reports <- cbind(fastQC=c(""),"fastQC.zip"=c(""),"species_diagrams"=c(""),"_diagrams.zip"=c(""),"isomiR-SEA"=c(""),xlsx=c(""),genomeless=c(""),multiQC=c(""))[-1,]
        for(i in exps){
            row<-c()
            for(j in c("_fastQC.html","_fastQC.zip","_species_diagrams.html","_sRNAflow_diagrams.zip","_isomiR-SEA.zip","_results.xlsx","_genomeless.xlsx","_multiqc.html")) 
